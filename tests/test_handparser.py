...
class TestDictBehavior:
    ...
    def test_can_get_key(self, all_hands):
        assert all_hands['ident']

    def test_object_is_iterable(self, all_hands):
        for attr in all_hands:
            assert attr in self.expected_keys

    def test_setting_key_reset_parsed_flags(self, all_hands):
        assert all_hands.parsed
        assert all_hands.header_parsed
        all_hands['game'] = 'OMAHA'
        assert not all_hands.parsed
        assert not all_hands.header_parsed

    def test_deleting_key_should_reset_parsed_flags(self, all_hands):
        assert all_hands.parsed
        assert all_hands.header_parsed
        del all_hands['players']
        assert not all_hands.parsed
        assert not all_hands.header_parsed

    def test_deleting_attribute_should_reset_parsed_flags(self, all_hands):
        assert all_hands.parsed
        assert all_hands.header_parsed
        del all_hands.button
        assert not all_hands.parsed
        assert not all_hands.header_parsed

    def test_changing_attribute_should_reset_raw_attribute(self, all_hands):
        assert all_hands.parsed
        all_hands.players = []
        assert all_hands.raw is None
